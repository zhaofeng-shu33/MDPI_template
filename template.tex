...
\end{Definition}
In physics, $\beta$ is referred to the inverse temperature and $Z_G(\gamma, \beta)$ is called the partition function.
...
The exact recovery error probability for $\hat{X}^*$ can be written as $P_e(\hat{X}^*) := \sum_{G \in \cG_n} P_G(G) P_{\sigma | G}(S^c_k(X))$. We can see from this expression that the error probability is determined
...
\end{enumerate}
\end{Theorem}
By simple calculus, $\tilde{g}(\beta) < 0$ for $\beta> \beta^*$ and $g(\beta)>0$ for $\beta < \beta^*$. $g(\bar{\beta}) < 0$ follows from \eqref{eq:abk}. The illustration of $g(\beta), \tilde{g}(\beta)$ is shown in Fig \ref{fig:g}.
