...
\abstract{In this paper, we study the phase transition property of an Ising model defined on a special random graph---the stochastic block model (SBM).
	...
	Additionally, we give an unbiased convergent estimator for the model parameters of the SBM, which can be computed in constant time.
...
semi-definite relaxation, spectral clustering, and label propagation, not only have theoretical guarantee when applied to the SBM,
...
For both cases, the~asymptotic behavior of detection errors
...
In this article, we will decouple the inference and recovery problems and propose an unbiased convergent estimator for SBM parameters when the number of communities is known. Once  the estimator is obtained, the~recovery condition can be checked to determine whether it is possible to recover the labels
...
property, and give the recovery error an upper bound. The~error bounds decay polynomially and quickly in different phases.
...
use the metropolis sampling technique to sample from the Ising model defined based on the  SBM. %confirm.
...
In this paper, we consider a special symmetric stochastic block model (SSBM), which is defined as follows:
	...
	\item The existence of each edge is independent of the other.
	\end{enumerate}
\end{Definition}
...
of $(a,b)$. That is,
$\mathbb{E}[\hat{a}] = a, \mathbb{E}[\hat{b}] = b$, and $\hat{a}$ and $\hat{b}$ converge to $a,b$ in probability, respectively.
\end{Theorem}
...
\begin{equation}\label{eq:energy}
	...
	The subscript in $P_{\sigma|G}$ indicates that the distribution depends on $G$, and~	$Z_G(\alpha,\beta)$ is the normalizing constant for this distribution.
\end{Definition}
...
by two parameters $(\gamma, \beta)$. When these parameters take proper values, $ P_e(\hat{X}^*)\to 0$ and the exact recovery of the SBM are achievable. On~the contrary, $P_e(\hat{X}^*) \to 1$ if $(\gamma, \beta)$ takes other values.
...
implies that $P_{\sigma | G}(\sigma = X)$ has the largest probability for almost the entire graph $G$. Therefore, instead of
...
for $\Dist(\sigma, X) \geq 1$. Then  union bound to used to sum these error probabilities up.
...
is equivalent with maximum likelihood or maximum modularity in an asymptotic case. The~following analysis shows
...
$$
where the parameter $\gamma$ in $H(\sigma)$ satisfies \mbox{$\gamma \frac{\log n}{n} = \frac{1}{\log(a/b)}(\log (1-\A) - \log (1-\B))$} \textls[-20]{and $C$ is a constant irrelevant with $\sigma$.
...
For some special Ising model, it needs to take $N=O(n\log n)$ to generate the sample for good approximation~\cite{mcmc}. For~our model, it is unknown whether $O(n\log n)$ is sufficient, and~we empirically chose $N=O(n^2)$ in numerical experiments.
...
We chose $m_1=2100,m_2=6000$, which is fairly large, to achieve a good approximation of $P_e(\hat{X}^*)$ by the law of large numbers.
...
The vertical red line indicating $\beta=\beta^* = 0.198$, computed from \eqref{eq:beta_star}, represents the phase transition threshold. The~point $(0.199,\frac{1}{2})$ in the figure
can be regarded as the empirical phase transition threshold, whose first coordinate is close to $\beta^*$.
The green line $(\beta, n^{g(\beta)/2})$ is the theoretical lower bound of accuracy for $\beta>\beta^*$, whereas the purple line
...
theoretical and empirical work will be done in the future, such as a convergence analysis on modularity (in Equation \eqref{eq:Qtransform}), the~necessary iteration time (in Algorithm \ref{alg:m}) for Metropolis sampling, and~so~on.
...
\end{proof}
The convergence of $|E|$ in the ER %define if appropriate.
graph can be extended directly to the SBM since the existence of each edge is independent.
However for $T$, it is a little tricky since the existence of each triangle is dependent on each other. The~following two lemmas
...
By taking expectation on both sizes of Equations  \eqref{eq:e_1} and \eqref{eq:e_2} we can show $\mathbb{E}[\hat{a}] = a,
...
\begin{align}
	...
	\end{align}
\end{Lemma}
The proof of Lemma \ref{lem:minus} is mainly composed out of careful counting techniques, and~we omit it here.
...
\begin{Lemma}\label{lem:sigmaX}
	...
	happens with a probability (w.r.t. %define is appropriate
...
where $\mathbf{1}_n$ is the \hl{all-in-one vector} %confirm.
...
\begin{Lemma}\label{lem:small}
	...
	happens with a probability (w.r.t. SSBM) less than $\exp(-\tau(\alpha,\beta) n \sqrt{\log  n} )$ where $C$ is an arbitrary constant, $\tau(\alpha,\beta)$ is a positive number.
\end{Lemma}
\begin{proof}
