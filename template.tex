...
\abstract{In this paper, we study the phase transition property of Ising model defined on a special random graph -- stochastic block model (SBM).
	Based on Ising model, we propose a stochastic estimator to achieve the exact recovery for SBM.
	...
	Besides, we give an unbiased convergent estimator for the model parameters of SBM, which can be computed in constant time.
	Finally, we use metropolis sampling to realize the stochastic estimator and verify the phase transition phenomenon by experiments.}
...
Stochastic Block Model (SBM) is one of the most commonly used statistical models for community detection problems  \cite{holland1983stochastic, abbe2017community}.
...
For both cases, the asymptotic behavior of detection error
...
Parameters inference on SBM is often considered alongside the exact recovery problem.
...
Ising model, which is a probability distribution of node states \cite{ising1925beitrag}. We use the terminology node states and node labels interchangably throughout this paper, both of which refer to the membership of the underlining community.
Ising model is originally proposed in statistical mechanics to model the ferromagnetism phenomenon but has wide application in neuroscience, information theory
...
property and give the recovery error upper bound. The error bounds decay polynomially fast in different phases.
...
Besides, searching the state with maximal probability
...
Derived from the Ising model, in section \ref{sec:em}, the energy minimization method is introduced, and we establish its connection with maximum likelihood and modularity
...
The connection between maximum modularity and maximal likelihood is investigated in \cite{newman2016equivalence}. To get an optimal value
...
Given $X$, the random graph $G$ is drawn under $\SSBM(n,k,p,q)$. We say that the exact recovery is solvable for $\SSBM(n,k,p,q)$ if there exists an algorithm that takes
...
exact recovery of label $X$ is possible by \eqref{eq:abk}. Besides,  Theorem \ref{thm:ab12} provides good estimation of $a,b$ to initialize their parameters of
...
the graph observation $G$ from node label $X$, Ising model provides a way to generate estimators of $X$ from $G$
by a stochastic procedure. The definition of such Ising model is given
as follows:
\begin{Definition}[Ising model with $k$ states]\label{def:ising}
	Given a graph $G$ sampled from $\SSBM(n,k,\A,\B)$, the Ising model with parameters $\gamma,\beta>0$
...
The classical definition of Ising model is specified by $H(\sigma)=-\sum_{(i,j)\in E(G)} \sigma_i \cdot \sigma_j$ for
$\sigma_i = \pm 1$.
...
Secondly, we allow the state at each node to take $k$ values from $W$ instead of two values $\pm 1$.
When $\gamma = 0$ and $k=2$, Definition \ref{def:ising}
is reduced to the classical definition of Ising model up to a scaling factor.

Definition \ref{def:ising} gives a stochastic estimator $\hat{X}^*$ for $X$: $\hat{X}^*$ is one sample generated from Ising model, which is denoted as
...
\end{enumerate}
\end{Theorem}
By simple calculus, $\tilde{g}(\beta) < 0$ for $\beta> \beta^*$ and $g(\beta)>0$ for $\beta < \beta^*$. $g(\bar{\beta}) < 0$ follows from \eqref{eq:abk}. The illustration of $g(\beta), \tilde{g}(\beta)$ is shown in Figure \ref{fig:g}.
...
\begin{subfigure}{0.5\textwidth}
	...
	The exact recovery of SSBM is solvable only in Region I.}\label{fig:pt}
\end{subfigure}
\caption{Illustration of Theorem \ref{thm:phase_transition} }
\end{figure}
...
implies that $P_{\sigma | G}(\sigma = X)$ has the largest probability for almost all graph $G$. Therefore, instead of
...
size $|\{v \in [n] : X_v = u\}| = \frac{n}{k}$ for each label $u$, another formulation is to restrict the search space to
...
for $\Dist(\sigma, X) \geq 1$. Then using union bound to sum these error probabilities up.
We note that a loose bound $n^{g(\bar{\beta})/4}$ is obtained in \cite{abbe2015exact} for the estimator $\hat{X}''$ when $k=2$.
...
$$
where the parameter $\gamma$ in $H(\sigma)$ satisfying $\gamma \frac{\log n}{n} = \frac{1}{\log(a/b)}(\log (1-\A) - \log (1-\B))$ and $C$ is a constant irrelevant with $\sigma$.
...
Using $a>b$ and the inequality $x-1>\log x > 2 \frac{x-1}{x+1}$ for $x>1$ we can verify that $\gamma_{MQ} > \gamma_{ML} > b$. That is, both maximum likelihood and maximum modularity estimator satisfy the exact recovery conditions $\gamma > b $ in Theorem \ref{thm:error_rate}.
...
Empirically speaking, starting from a random state, Metropolis algorithm updates the state by randomly selecting one position to flip its state at each iteration step.
Then after some initial burning time, the generated samples can be regarded as sampling from Ising model.

The theoretical guarantee of Metropolis sampling is based on Markov chain. Under some general conditions, Metropolis samples converge to
the steady state of the Markov chain, and the steady state follows the probability distribution to be approximated. For Ising model, there are many previous works which have shown that the convergence of Metropolis sampling \cite{diaconis1998we}.
...
In the remaining part of this section, we conduct experiments to verify our theoretical results.
Firstly, we consider several combinations of $(a,b,k)$ and obtain the estimator $(\hat{a}, \hat{b})$ by Theorem \ref{thm:ab12}. Using
the empirical mean squared error (MSE) $\frac{1}{m} \sum_{i=1}^m (\hat{a}-a)^2 + (\hat{b}-b)^2$ as the criterion
and choosing $m=1000$, the result is shown in Figure \ref{fig:estimator}. As we can see, as $n$ increases, MSE decreases polynomially fast. Therefore, the convergence
of $\hat{a} \to a$ and $\hat{b} \to b$ is verified.

Seconly, using Metropolis sampling, we conduct a moderate simulation to verify Theorem \ref{thm:phase_transition} for the case $\gamma > b$.
We choose $n=9000, k=2$, and the empirical accuracy is computed by $P_e = \frac{1}{m_1m_2}\sum_{i=1}^{m_1} \sum_{j=1}^{m_2} \mathbbm{1}[\hat{X}^* = \pm X]$.
In this formula, $m_1$ is the number of times to generate the random graph by SBM
while $m_2$ is the number of times to generate consecutive samples by Algorithm \ref{alg:m} for a given graph.
We choose $m_1=2100,m_2=6000$, which is fairly large to make good approximation of $P_e(\hat{X}^*)$ by law of large numbers.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\authorcontributions{The work of this paper is conceptualized by Min Ye, who also developed the mathematical analysis methodology for the proof of main
	theorem of this paper. All simulation code is written by Feng Zhao, who also extended the idea of Min Ye, concreted the proof and wrote this paper.
	The work is supervised and funded by Shao-Lun Huang. All authors have read and agreed to the published version of the manuscript.}
