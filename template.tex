...
implies that $P_{\sigma | G}(\sigma = X)$ has the largest value for almost all graphs $G$ generated by the SBM. Therefore, instead of
...
\begin{proof}
	...
	$\mathbb{E}[|E|] = \frac{n(n-1)}{2}p = \frac{(n-1)\log n}{2}a$ and $\Var[|E|] = \frac{n(n-1)}{2} p(1-p) < a\frac{(n-1)\log n}{2}$.
...
\begin{Lemma}\label{lem:sbmV}
	For $\SSBM(n, k, p, q)$ where $p=\frac{a\log n}{n}, q = \frac{b\log n}{n}$. The~number of triangles is $T$.
...
\begin{proof}
	...
	The second is the number of triangles that have one node in community $i$ and its corresponding edge in community $j$, $T_{ij}$. There are $k(k-1)$ terms of $T_{ij}$. The~third is the number of triangles that have one node in community $i$, one node in community $j$ and one node in community $k$.
