...
The exact recovery error probability for $\hat{X}^*$ can be written as $P_e(\hat{X}^*) := \sum_{G \in \cG_n} P_G(G) P_{\sigma | G}(S^c_k(X))$. We can see from this expression that error probability is determined
...
\begin{subfigure}{0.5\textwidth}
	...
	\caption{Phase transition region with respect to $(\beta, \gamma)$ plane.
